...
Then swap `public/img/avatar.svg` for a real photo (keep the filename, or update the path in `src/components/Hero.astro`) and drop a 1200x630 `og.jpg` into `public/img/` for social share cards. Finally, set your domain as `site` in `astro.config.mjs`.
...
Create `src/content/trips/my-trip.md` with a title, place, date, summary, a hero image, and a `gallery` list pointing at images in `public/img/trips/...`. You get the photo carousel and lightbox for free.
